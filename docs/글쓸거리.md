...
git reflog show flowers flowers@{@}




-- SSL 통신을 위한 인증서 만들기
openssl req -x509 -out smartky.kyu.ac.kr.pem -keyout smartky.kyu.ac.kr-key.pem -newkey rsa:2048 -nodes -sha256 -days 365 -subj "/CN=smartky.kyu.ac.kr"

openssl x509 -in smartky.kyu.ac.kr.pem -text -noout


-- 루트 인증서 만들기 시작
openssl genrsa -out smartky.kyu.ac.kr.ca.key
openssl req -x509 -new -nodes -key smartky.kyu.ac.kr.ca.key -sha256 -days 3650 -out smartky.kyu.ac.kr.ca.pem
  Common name : smartky.kyu.ac.kr

-- csr 생성
openssl req -new -key smartky.kyu.ac.kr-key.pem -out smartky.kyu.ac.kr.csr -config san.cnf

-- 서명
openssl x509 -req -in smartky.kyu.ac.kr.csr -CA smartky.kyu.ac.kr.ca.pem -CAkey smartky.kyu.ac.kr.ca.key -CAcreateserial -out smartky.kyu.ac.kr-signed.pem -days 365 -sha256 -extfile san.cnf -extensions v3_req

-- 체인 구성방법
cat smartky.kyu.ac.kr-signed.pem smartky.kyu.ac.kr.ca.pem > fullchain.pem
copy /b smartky.kyu.ac.kr-signed.pem + smartky.kyu.ac.kr.ca.pem fullchain.pem

-- 윈도우 루트 인증기관 관련 변환환
openssl x509 -outform der -in smartky.kyu.ac.kr.ca.pem -out smartky.kyu.ac.kr.ca.crt
openssl s_client -connect smartky.kyu.ac.kr:8443 -servername smartky.kyu.ac.kr -CAfile smartky.kyu.ac.kr.ca.pem

openssl x509 -noout -modulus -in smartky.kyu.ac.kr-signed.pem | openssl md5

openssl rsa -noout -modulus -in smartky.kyu.ac.kr-key.pem | openssl md5


openssl x509 -req -in smartky.kyu.ac.kr.csr -CA smartky.kyu.ac.kr.ca.pem -CAkey smartky.kyu.ac.kr.ca.key -CAcreateserial -out smartky.kyu.ac.kr-signed.pem -days 365 -sha256

openssl pkcs12 -export -in smartky.kyu.ac.kr-signed.pem -inkey smartky.kyu.ac.kr-key.pem -out keystore.p12 -name smartky -CAfile smartky.kyu.ac.kr.ca.pem -caname root -password pass:123456
...
X509v3 extensions:
  ...
  X509v3 Subject Alternative Name:
      DNS:smartky.kyu.ac.kr, DNS:www.smartky.kyu.ac.kr
  X509v3 Subject Key Identifier:
      6C:AD:A3:DE:98:D8:68:3A:B4:D5:D9:23:46:DA:B1:AC:28:6B:2E:79
  X509v3 Authority Key Identifier:
      91:F7:C5:D2:00:FE:42:67:39:90:CF:08:13:67:80:00:E1:35:01:6B

Digital Signature 가 누락되었단다...
[ req ]
default_bits        = 2048
...
commonName_default          = smartky.kyu.ac.kr
...
[ alt_names ]
DNS.1 = smartky.kyu.ac.kr
DNS.2 = www.smartky.kyu.ac.kr

openssl req -new -key smartky.kyu.ac.kr-key.pem -out smartky.kyu.ac.kr.csr -config openssl.cnf

openssl x509 -req -in smartky.kyu.ac.kr.csr -CA smartky.kyu.ac.kr.ca.pem -CAkey smartky.kyu.ac.kr.ca.key -CAcreateserial -out smartky.kyu.ac.kr-signed.pem -days 365 -sha256 -extensions v3_ca -extfile openssl.cnf

cat smartky.kyu.ac.kr-signed.pem smartky.kyu.ac.kr.ca.pem > fullchain.pem

san.cnf : openssl 설정 파일

smartky.kyu.ac.kr.ca.pem : 자체 서명 루트 인증서
smartky.kyu.ac.kr.ca.key : 자체 서명 루트 인증서 개인키

smartky.kyu.ac.kr.pem : 서버 인증서
smartky.kyu.ac.kr-key.pem : 서버 인증서 개인키

smartky.kyu.ac.kr.csr : 서명 요청 파일 
smartky.kyu.ac.kr-signed.pem : 자체 서명 루트 인증서로 서명된 서버 인증서.

fullchain.pem : 자체서명 루트 인증서, 자체 서명 루트 인증서 키 체인

spring boot https 설정 관련 : smartky.kyu.ac.kr.ca.pem 를 신뢰할 수 있는 루트 인증 기관으로 등록
